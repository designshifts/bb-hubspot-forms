(function () {
  function setResult(message, isSuccess) {
    var result = document.getElementById("bb-forms-for-hubspot-test-result");
    if (!result) {
      return;
    }
  ...
  function handleClick() {
    if (!window.bbFormsForHubspotSettings) {
      return;
    }
    ...
    var button = document.getElementById("bb-forms-for-hubspot-test-connection");
    if (button) {
      button.disabled = true;
    }
    setResult("Testing connection...", true);

    fetch(window.bbFormsForHubspotSettings.restUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-WP-Nonce": window.bbFormsForHubspotSettings.nonce,
      },
      body: JSON.stringify({
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var toggleCaptchaFields = function () {
      ...
      var captchaFields = document.querySelectorAll(
        ".bb-forms-for-hubspot-captcha-field"
      );
      var recaptchaFields = document.querySelectorAll(
        ".bb-forms-for-hubspot-recaptcha-field"
      );
