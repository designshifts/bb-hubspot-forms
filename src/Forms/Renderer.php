...
final class Renderer {
	...
	public static function register_assets(): void {
		...
		wp_add_inline_script(
			'bb-forms-for-hubspot-frontend',
			'window.bbFormsForHubspotConfig = ' . wp_json_encode( $config ) . ';',
			'before'
		);
	...
	public static function render_shortcode( array $atts ): string {
		...
		if ( ! $form_id ) {
			return self::render_admin_notice( __( 'HubSpot form ID is missing.', 'bb-forms-for-hubspot' ) );
		}
		...
		if ( empty( $schema ) || empty( $schema['fields'] ) ) {
			return self::render_admin_notice( __( 'Form schema is missing or invalid.', 'bb-forms-for-hubspot' ) );
		}
		...
		if ( 'recaptcha_v3' === $captcha_provider && $captcha_site_key ) {
			wp_enqueue_script(
				'bb-forms-for-hubspot-recaptcha',
		...
		wp_enqueue_script( 'bb-forms-for-hubspot-frontend' );

		// Enqueue default styles if enabled.
		if ( Settings::get( 'enable_default_styles', true ) ) {
			wp_enqueue_style( 'bb-forms-for-hubspot-frontend' );
		}
		...
		return sprintf(
			'<form class="bb-forms-for-hubspot-form" method="post" novalidate%1$s>%2$s%3$s<button type="submit">%4$s</button><div class="bb-forms-for-hubspot-form__message" aria-live="polite"></div></form>',
			$attributes,
			$fields_html,
			$consent_html,
			esc_html__( 'Submit', 'bb-forms-for-hubspot' )
		);
	}
	...
	private static function render_consent_block( string $consent_mode ): string {
		...
		if ( empty( $consent_text ) ) {
			$consent_text = __( 'I agree to allow this website to store and process my personal data.', 'bb-forms-for-hubspot' );
		}

		$output = '<div class="bb-forms-for-hubspot-form__consent">';

		// Required: Data Processing Consent.
		$output .= '<label class="bb-forms-for-hubspot-form__consent-field">';
	...
	private static function render_fields( array $fields, array $overrides ): string {
		...
		foreach ( $ordered as $field ) {
			...
			$output .= '<label class="bb-forms-for-hubspot-form__field">';
			$output .= '<span class="bb-forms-for-hubspot-form__label">' . $label . '</span>';

			if ( 'select' === $type && ! empty( $options ) ) {
				$output .= '<select name="' . esc_attr( $name ) . '"' . ( $required ? ' required' : '' ) . '>';
				$output .= '<option value="">' . esc_html__( '— Select —', 'bb-forms-for-hubspot' ) . '</option>';
				...
				foreach ( $options as $option ) {
					$output .= '<label class="bb-forms-for-hubspot-form__option">';
	...
	private static function render_admin_notice( string $message ): string {
		...
		return '<div class="bb-forms-for-hubspot-form__notice">' . esc_html( $message ) . '</div>';
	}
}
