...
(function() {
	...
	registerBlockType("bb-hubspot-forms/form-config", {
		title: __("HubSpot Form Config", "bb-hubspot-forms"),
		icon: "feedback",
		category: "widgets",
		supports: {
			html: false,
		...
		edit: function(props) {
				...
				apiFetch({ path: "/bb-hubspot/v1/forms" })
					.then(function(response) {
						if (response.success && response.forms) {
							...
							setError(response.error || __("Unable to load forms.", "bb-hubspot-forms"));
						}
					})
					.catch(function(err) {
						var msg = (err && err.error) || (err && err.message) || __("Unable to load forms.", "bb-hubspot-forms");
						setError(msg);
					})
					.finally(function() {
						setLoading(false);
				...
				apiFetch({
					...
					.then(function(response) {
						if (response.success && response.schema) {
							...
							setError(response.error || __("Unable to fetch schema.", "bb-hubspot-forms"));
						}
					})
					.catch(function(err) {
						var msg = (err && err.error) || (err && err.message) || __("Unable to fetch schema.", "bb-hubspot-forms");
						setError(msg);
					})
					.finally(function() {
						setSyncing(false);
			...
			var formOptions = [{ value: "", label: __("— Select a HubSpot form —", "bb-hubspot-forms") }];
			forms.forEach(function(f) {
				formOptions.push({ value: f.id, label: f.name || f.id });
			});

			var lastSynced = fetchedAt ? new Date(fetchedAt * 1000).toLocaleString() : __("Never", "bb-hubspot-forms");
			...
			var fieldRows = orderedFields.map(function(field, index) {
				...
				return el("div", {
					...
					el("div", { style: { display: "flex", flexDirection: "column", gap: "2px" } },
						el(Button, { isSmall: true, icon: "arrow-up-alt2", label: __("Move up", "bb-hubspot-forms"), onClick: function() { moveField(index, -1); }, disabled: index === 0 }),
						el(Button, { isSmall: true, icon: "arrow-down-alt2", label: __("Move down", "bb-hubspot-forms"), onClick: function() { moveField(index, 1); }, disabled: index === orderedFields.length - 1 })
					),
					el("div", { style: { flex: 1, minWidth: 0 } },
						el(TextControl, {
							label: __("Label", "bb-hubspot-forms"),
						...
						el("div", { style: { fontSize: "12px", color: "#666", marginTop: "4px", display: "flex", gap: "8px", flexWrap: "wrap" } },
							...
							field.required && el("span", { style: { background: "#d63638", color: "#fff", padding: "2px 8px", borderRadius: "12px" } }, __("Required", "bb-hubspot-forms"))
						)
					),
					el(ToggleControl, {
						label: __("Show", "bb-hubspot-forms"),
						checked: !hidden,
			...
			return el("div", { style: containerStyle },
				el("div", { style: gridStyle },
					...
					el("div", { style: cardStyle },
						el("div", { style: cardHeaderStyle }, __("HubSpot Form", "bb-hubspot-forms")),
						el("div", { style: cardBodyStyle },
							el("p", { style: { color: "#666", marginTop: 0 } },
								__("Select a form from your HubSpot account to mirror in WordPress.", "bb-hubspot-forms")
							),
							...
							el("div", { style: { marginTop: "12px", display: "flex", alignItems: "center", gap: "12px" } },
								el(Button, {
									...
									isBusy: syncing,
								}, syncing ? __("Syncing…", "bb-hubspot-forms") : __("Sync fields from HubSpot", "bb-hubspot-forms")),
								el("span", { style: { color: "#666", fontSize: "12px" } }, __("Last synced:", "bb-hubspot-forms") + " " + lastSynced)
							),
					...
					// Embed Card
					el("div", { style: cardStyle },
						el("div", { style: cardHeaderStyle }, __("Embed", "bb-hubspot-forms")),
						el("div", { style: cardBodyStyle },
							el("p", { style: { color: "#666", marginTop: 0 } },
								__("Use this shortcode to embed the form on any page or post.", "bb-hubspot-forms")
							),
							!postId 
								? el("p", { style: { fontStyle: "italic", color: "#666" } }, __("Save draft to generate shortcode.", "bb-hubspot-forms"))
								: el(Fragment, {},
									el(TextControl, {
										value: shortcode,
										readOnly: true,
									...
									el(Button, {
										...
										onClick: handleCopy,
										style: { marginTop: "8px" },
									}, copied ? __("Copied!", "bb-hubspot-forms") : __("Copy shortcode", "bb-hubspot-forms"))
								)
						)
					),
					...
					el("div", { style: fullWidthCardStyle },
						el("div", { style: cardHeaderStyle }, __("Fields", "bb-hubspot-forms")),
						el("div", { style: cardBodyStyle },
							el("p", { style: { color: "#666", marginTop: 0 } },
								__("Reorder fields, rename labels, or hide fields. HubSpot property names and required rules are synced from HubSpot.", "bb-hubspot-forms")
							),
							!schemaFields.length
								? el("p", { style: { fontStyle: "italic", color: "#888", textAlign: "center", padding: "24px" } },
									__("Select a HubSpot form and sync to load fields.", "bb-hubspot-forms")
								)
								: el("div", {}, fieldRows)
						)
					)
