...
final class Client {
	...
	public static function list_forms(): array {
		...
		if ( empty( $private_token ) ) {
			...
			return array(
				'success' => false,
				'error'   => 'HubSpot Private App Token is not configured. Go to Settings → BetterBuilds Forms for HubSpot.',
			);
		}
	...
	public static function submit_form( string $hubspot_form_id, array $fields, array $context = array(), array $consent = array() ): array {
		...
		if ( empty( $portal_id ) ) {
			return array(
				'success' => false,
				'error'   => 'HubSpot Portal ID is not configured. Go to Settings → BetterBuilds Forms for HubSpot.',
			);
		}
